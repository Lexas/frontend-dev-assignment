...
this.data[s];if(m){var g=r.arrayEntry(l,s-1),y=r.arrayEntry(h,s),b=m.CHTML,w=m.CHTMLspanElement
();w.style.verticalAlign=y;var E=m.type==="mlabeledtr"?1:0;for(u=0,a=m.data.length
;u<a-E;u++){var S=m.data[u+E];if(S){var x=r.arrayEntry(f,u-1),T=r.arrayEntry(c,u)
,N=S.CHTMLspanElement();u&&(b.w+=x,N.style.paddingLeft=r.Em(x)),s&&(N.style.paddingTop=
r.Em(g)),N.style.textAlign=T}}d+=b.h+b.d,s&&(d+=g),b.w>v&&(v=b.w)}}var C=this.CHTML
;return C.w=v,C.h=d/2+.25,C.d=d/2-.25,C.l=C.r=.125,e}}),i.mlabeledtr.Augment({CHTMLdefaultSpan
