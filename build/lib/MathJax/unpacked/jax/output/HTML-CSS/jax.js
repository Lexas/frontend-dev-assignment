...
:function(e){var n=e.jax[this.id],r,i=n.length,s,o,f,l,c,h,p,d,v,m,g=!1,y,b=this.
config.linebreaks.automatic,w=this.config.linebreaks.width;b?(g=w.match(/^\s*(\d+(\.\d*)?%\s*)?container\s*$/
)!=null,g?w=w.replace(/\s*container\s*/,""):m=this.defaultWidth,w===""&&(w="100%"
...
d,this.em=d*v/100,h.HTMLCSS.ex=p,h.HTMLCSS.cwidth=y/this.em,h.HTMLCSS.lineWidth=b?
this.length2em(w,1,m/this.em):1e6}for(r=0;r<i;r++){s=n[r];if(!s.parentNode)continue;
...
;var p=(e.root.id||"MathJax-Span-"+e.root.spanID)+"-zoom",d=document.getElementById
(p).firstChild;while(d&&d.style.width!==h)d=d.nextSibling;if(d){var v=d.offsetWidth
;d.style.width="100%",v>o&&(i.style.width=v+100+"px")}}d=i.firstChild.firstChild.
style,c.H!=null&&c.H>c.h&&(d.marginTop=n.Em(c.H-Math.max(c.h,n.FONTDATA.lineH))),
c.D!=null&&c.D>c.d&&(d.marginBottom=n.Em(c.D-Math.max(c.d,n.FONTDATA.lineD))),c.lw<0&&
(d.paddingLeft=n.Em(-c.lw)),c.rw>c.w&&(d.marginRight=n.Em(c.rw-c.w)),i.style.position="absolute"
,h||(s.style.position="absolute");var m=i.offsetWidth,g=i.offsetHeight,y=s.offsetHeight
,b=s.offsetWidth;return i.style.position=s.style.position="",{Y:-u.getBBox(i).h,mW
:b,mH:y,zW:m,zH:g}},initImg:function(e){},initHTML:function(e,t){},initFont:function(
...
.em,c=o.H==null?o.h:o.H,h=o.D==null?o.d:o.D,p=a-c-l,d=a+h+l,v=-1e3,m=1e3;e.style.
clip="rect("+this.Em(p)+" "+this.Em(m)+" "+this.Em(d)+" "+this.Em(v)+")"}}e.style
...
f.stretch){f.HW[l][2]&&(o*=f.HW[l][2]),f.HW[l][3]&&(i=f.HW[l][3]);var h=this.addElement
(n,"span");this.createChar(h,[i,f.HW[l][1]],o,u),n.bbox=h.bbox,n.offset=.65*n.bbox
...
var g=t["offset"+m[d].offset];if(g&&s>=m[d].low&&s<=m[d].high){m[d].remap&&m[d].remap
[s]?s=g+m[d].remap[s]:(s=s-m[d].low+g,m[d].add&&(s+=m[d].add)),t["variant"+m[d].offset
...
)&&!o[5].img){i.length&&(this.addText(f,i),i="");var y=!!f.style.fontFamily||!!e.
style.fontStyle||!!e.style.fontWeight||!u.directory||l;l=!1,f!==e&&(y=!this.checkFont
(u,e.style),f=e),y&&(f=this.addElement(e,"span",{isMathJax:!0,subSpan:!0})),this.
...
Math.max(0,n.getW(e)+h),d=o.h+o.d,v=-o.d,m=0,g=0;p>0&&(p+=2*u,a-=u),d>0&&(d+=2*u,
v-=u),f=-p-a,i&&(f-=i.right,v-=i.bottom,m+=i.left,g+=i.right,o.h+=i.top,o.d+=i.bottom
,o.w+=i.left+i.right,o.lw-=i.left,o.rw+=i.right),s&&(d+=s.top+s.bottom,p+=s.left+
s.right,f-=s.right,v-=s.bottom,m+=s.left,g+=s.right,o.h+=s.top,o.d+=s.bottom,o.w+=
s.left+s.right,o.lw-=s.left,o.rw+=s.right),g&&(e.style.paddingRight=n.Em(g));var y=
...
(d),verticalAlign:n.Em(v),marginLeft:n.Em(a),marginRight:n.Em(f)}});return n.setBorders
(y,i),o.width&&(y.style.width=o.width,y.style.marginRight="-"+o.width),n.msieInlineBlockAlignBug&&
(y.style.position="relative",y.style.width=y.style.height=0,y.style.verticalAlign=
y.style.marginLeft=y.style.marginRight="",y.style.border=y.style.padding="",i&&n.
msieBorderWidthBug&&(d+=i.top+i.bottom,p+=i.left+i.right),y.style.width=n.Em(m+u)
,n.placeBox(n.addElement(y,"span",{noAdjust:!0,isMathJax:!0,style:{display:"inline-block"
,position:"absolute",overflow:"hidden",background:t.mathbackground||"transparent"
,width:n.Em(p),height:n.Em(d)}}),a,o.h+u),n.setBorders(y.firstChild,i)),e.parentNode
.insertBefore(y,e),n.msieColorPositionBug&&(e.style.position="relative"),y}return null
...
.TeX.axis_height*this.scale*this.mscale,d=(e.bbox.h-e.bbox.d)/2-p;n.safariVerticalAlignBug&&
e.lastChild.nodeName==="IMG"?e.lastChild.style.verticalAlign=n.Em(n.unEm(e.lastChild
.style.verticalAlign||0)/n.em-d/e.scale):n.konquerorVerticalAlignBug&&e.lastChild
.nodeName==="IMG"?(e.style.position="relative",e.lastChild.style.position="relative"
,e.lastChild.style.top=n.Em(d/e.scale)):e.style.verticalAlign=n.Em(-d/e.scale),e.
bbox.h-=d,e.bbox.d+=d,e.bbox.rw>e.bbox.w&&(e.bbox.ic=e.bbox.rw-e.bbox.w,n.createBlank
...
bbox.w),p=n.thickness2em(s.linethickness,this.scale)*this.mscale,d,v,m,g,y=n.TeX.
min_rule_thickness/this.em;u?(m=n.TeX.num1,g=n.TeX.denom1):(m=p===0?n.TeX.num3:n.
TeX.num2,g=n.TeX.denom2),m*=o,g*=o;if(p===0)d=Math.max((u?7:3)*n.TeX.rule_thickness
,2*y),v=m-r.bbox.d-(i.bbox.h-g),v<d&&(m+=(d-v)/2,g+=(d-v)/2);else{d=Math.max((u?2
:0)*y+p,p/2+1.5*y),v=m-r.bbox.d-(a+p/2),v<d&&(m+=d-v),v=a-p/2-(i.bbox.h-g),v<d&&(
g+=d-v);var b=n.createBox(t);n.createRule(b,p,0,h+2*p),n.placeBox(b,0,a-p/2)}n.alignBox
(r,s.numalign,m),n.alignBox(i,s.denomalign,-g)}return this.HTMLhandleSpace(e),this
...
.d>l&&(f=(s.bbox.h+s.bbox.d-(l-u))/2);var d=n.FONTDATA.DELIMITERS[n.FONTDATA.RULECHAR
];return!d||c<d.HW[0][0]*o||o<.75?n.createRule(i,0,u,c):n.createDelimiter(i,n.FONTDATA
...
));n.MeasureSpans(f);var d=-n.BIGDIMEN,v=d;for(h=0,p=this.data.length;h<p;h++)this
.data[h]&&(a[h].bbox.w>v&&(v=a[h].bbox.w),!l[h]&&v>d&&(d=v));i==null&&t!=null?d=t
:d==-n.BIGDIMEN&&(d=v);for(h=v=0,p=this.data.length;h<p;h++)this.data[h]&&(c=a[h]
,l[h]&&(c.bbox=this.data[h].HTMLstretchH(c,d).bbox,h!==this.base&&(f[h].style.paddingLeft=
f[h].style.paddingRight="")),c.bbox.w>v&&(v=c.bbox.w));var m=n.TeX.rule_thickness*
this.mscale,g=n.FONTDATA.TeX_factor,y=a[this.base]||{bbox:this.HTMLzeroBBox()},b,
w,E,S,x,T,N,C=0;y.bbox.ic&&(C=1.3*y.bbox.ic+.05);for(h=0,p=this.data.length;h<p;h++
)if(this.data[h]!=null){c=a[h],x=n.TeX.big_op_spacing5*o;var k=h!=this.base&&s[this
.ACCENTS[h]];k&&c.bbox.w<=1/n.em+1e-4&&(c.bbox.w=c.bbox.rw-c.bbox.lw,c.bbox.noclip=!0
,c.bbox.lw&&c.insertBefore(n.createSpace(c.parentNode,0,0,-c.bbox.lw),c.firstChild
),n.createBlank(c,0,0,c.bbox.rw+.1)),T={left:0,center:(v-c.bbox.w)/2,right:v-c.bbox
.w}[s.align],b=T,w=0,h==this.over?(k?(N=Math.max(m*o*g,2.5/this.em),x=0,y.bbox.skew&&
(b+=y.bbox.skew,e.bbox.skew=y.bbox.skew,b+c.bbox.w>v&&(e.bbox.skew+=(v-c.bbox.w-b
)/2))):(E=n.TeX.big_op_spacing1*o*g,S=n.TeX.big_op_spacing3*o*g,N=Math.max(E,S-Math
.max(0,c.bbox.d))),N=Math.max(N,1.5/this.em),b+=C/2,w=y.bbox.h+c.bbox.d+N,c.bbox.
h+=x):h==this.under&&(k?(N=3*m*o*g,x=0):(E=n.TeX.big_op_spacing2*o*g,S=n.TeX.big_op_spacing4*
o*g,N=Math.max(E,S-c.bbox.h)),N=Math.max(N,1.5/this.em),b-=C/2,w=-(y.bbox.d+c.bbox
.h+N),c.bbox.d+=x),n.placeBox(c,b,w)}return this.HTMLhandleSpace(e),this.HTMLhandleColor
...
.w+=c,p.bbox.rw=Math.max(p.bbox.w,p.bbox.rw)),n.placeBox(f,0,0);var d=i;h?d=this.
data[this.sup].HTMLgetScale():p&&(d=this.data[this.sub].HTMLgetScale());var v=n.TeX
.sup_drop*d,m=n.TeX.sub_drop*d,g=f.bbox.h-v,y=f.bbox.d+m,b=0,w;f.bbox.ic&&(f.bbox
.w-=f.bbox.ic,b=1.3*f.bbox.ic+.05),this.data[this.base]&&t==null&&r==null&&(this.
...
(g=y=0);var E=this.getValues("subscriptshift","superscriptshift");E.subscriptshift=
E.subscriptshift===""?0:n.length2em(E.subscriptshift,s),E.superscriptshift=E.superscriptshift===""?0
:n.length2em(E.superscriptshift,s);if(!h)p&&(y=Math.max(y,n.TeX.sub1*i,p.bbox.h-.8*
l,E.subscriptshift),n.placeBox(p,f.bbox.w,-y,p.bbox));else if(!p)u=this.getValues
("displaystyle","texprimestyle"),w=n.TeX[u.displaystyle?"sup1":u.texprimestyle?"sup3"
:"sup2"],g=Math.max(g,w*i,h.bbox.d+.25*l,E.superscriptshift),n.placeBox(h,f.bbox.
w+b,g,h.bbox);else{y=Math.max(y,n.TeX.sub2*i);var S=n.TeX.rule_thickness*i;g-h.bbox
.d-(p.bbox.h-y)<3*S&&(y=3*S-g+h.bbox.d+p.bbox.h,v=.8*l-(g-h.bbox.d),v>0&&(g+=v,y-=
v)),n.placeBox(h,f.bbox.w+b,Math.max(g,E.superscriptshift)),n.placeBox(p,f.bbox.w
,-Math.max(y,E.subscriptshift))}return this.HTMLhandleSpace(e),this.HTMLhandleColor
...
width=o.style.width="100%");var d=this.HTMLhandleColor(e);f&&n.createRule(e,(f.bbox
.h+h)*p,(f.bbox.d+h)*p,0);if(!this.isMultiline&&this.Get("display")==="block"&&e.
bbox.width==null){var v=this.getValues("indentalignfirst","indentshiftfirst","indentalign"
,"indentshift");v.indentalignfirst!==r.INDENTALIGN.INDENTALIGN&&(v.indentalign=v.
indentalignfirst),v.indentalign===r.INDENTALIGN.AUTO&&(v.indentalign=this.displayAlign
),v.indentshiftfirst!==r.INDENTSHIFT.INDENTSHIFT&&(v.indentshift=v.indentshiftfirst
),v.indentshift==="auto"&&(v.indentshift="0");var m=n.length2em(v.indentshift,1,n
.scale*n.cwidth);if(this.displayIndent!=="0"){var g=n.length2em(this.displayIndent
,1,n.scale*n.cwidth);m+=v.indentalign===r.INDENTALIGN.RIGHT?-g:g}i.style.textAlign=
v.indentalign,m&&(m*=n.em/n.outerEm,t.Insert(e.style,{left:{marginLeft:n.Em(m)},right
:{marginLeft:n.Em(Math.max(0,e.bbox.w+m)),marginRight:n.Em(-m)},center:{marginLeft
:n.Em(m),marginRight:n.Em(-m)}}[v.indentalign]),d&&(d.style.marginLeft=n.Em(parseFloat
(d.style.marginLeft)+m),d.style.marginRight=n.Em(parseFloat(d.style.marginRight)-
m+(v.indentalign==="right"?Math.min(0,e.bbox.w+m)-e.bbox.w:0))))}}return e},HTMLspanElement
